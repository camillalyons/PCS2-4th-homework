...
#from all the words from the dictionary, we are picking only the ones with the length between the starting word (w) and the final word (v)
rightWords = []
def selectingWords(w, v):
    for word in dictionaryList:
        if len(w) >= len(v):
            if len(word) <= len(w) and len(word) >= len(v):
